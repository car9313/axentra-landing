...
```
?locale= → header país del hosting (cf-ipcountry, x-country,
...
| Archivo | Qué cambiar |
|---|---|
| `domain/locale.config.ts` | `SUPPORTED_LOCALES` (idiomas y países de tu público) y `LOCALE_MAP` (país→locale regional, p. ej. `BR: "pt-BR"`) |
...
async function resolveServerLocale(): Promise<LocaleId> {
  ...
  if (headerLocale && SUPPORTED_LOCALES.some((l) => l.id === headerLocale)) {
    return headerLocale as LocaleId;
  }
  const cookieLocale = (await cookies()).get(COOKIE_KEY)?.value;
  if (cookieLocale && SUPPORTED_LOCALES.some((l) => l.id === cookieLocale)) {
...
- `generateMetadata()` → title/description por locale (`getMetadataForLocale`).
...
2. Geo del hosting: headers de país (`cf-ipcountry`, `x-country`, cloudfront, cdn) → `LOCALE_MAP` → si no hay traducción regional, `fallbackLocaleForCountry` (idioma del país).
...
> `app/layout.tsx` (`resolveServerLocale`) lee **el header del proxy ANTES que la
> cookie**: así el SSR respeta la decisión fresca de la geo en cada request y no
> pinta idiomas viejos de la cookie al cambiar de país.

### 6.3. `LocaleProvider.tsx` (cliente)

- Contexto expone `{ resolvedLocale, isReady, setPreference }`.
- Init (una vez): aplica un **provisional** para pintar al instante (`?locale=` → `<html lang>` SSR → cookie → navigator → default), y **siempre** ejecuta `detectLocaleFromGeo()` en paralelo:
